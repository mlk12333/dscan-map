...
//        }
    ...
    if(core_bm_ != NULL) delete core_bm_;
}
...
void Graph::getTti(){
    for(int i = 0; i < n; i++){
        ...
        HashSet hash(out_pstart[i+1] - out_pstart[i], 1);
        for(unsigned int j = out_pstart[i]; j<out_pstart[i+1]; j++){
            out_vertex a;
            a.value =j;
            a.vertex = out_edges[j];
            hash.insert(a);
        }
        for(unsigned int j = out_pstart[i]; j<out_pstart[i+1]; j++){
            unsigned int v = out_edges[j];
        ...
        for(unsigned int j = in_pstart[i]; j<in_pstart[i+1]; j++){
            ...
            if(degree[i]/degree[v]>100||degree[v]/degree[i]>100) continue;
            if(hash.find(v).vertex) continue;
            if(((out_pstart[v+1] - out_pstart[v]) < (out_pstart[i+1] - out_pstart[i])) || (((out_pstart[v+1] - out_pstart[v]) == (out_pstart[i+1] - out_pstart[i])) && (i<v))){
                for(unsigned int k = out_pstart[v]; k<out_pstart[v+1]; k++){
                    unsigned int w = out_edges[k];
                    if(w == i) continue;
...
void Graph::getTti_cycle(){
//    for(int i = 0; i < n; i++){
//        int out_size = out_pstart[i+1] - out_pstart[i];
//        int in_size = in_pstart[i+1] - in_pstart[i];
//        if(out_size + in_size == 0) continue;
//        HashSet out_hash(out_size);
//        HashSet in_hash(in_size);
//        if(out_size>0){
//            for(unsigned int j = out_pstart[i]; j<out_pstart[i+1]; j++){
//                out_hash.insert(out_edges[j]);
//            }
//        }
//        if(in_size>0){
//            for(unsigned int j = in_pstart[i]; j<in_pstart[i+1]; j++){
//                in_hash.insert(in_edges[j]);
//            }
//        }
//
//        for(unsigned int j = out_pstart[i]; j<out_pstart[i+1]; j++){
//            unsigned int v = out_edges[j];
//            if(((out_pstart[v+1] - out_pstart[v]) < in_size) || (((out_pstart[v+1] - out_pstart[v]) == in_size) && (i<v))){
//
//                for(unsigned int k = out_pstart[v]; k<out_pstart[v+1]; k++){
//
//                    unsigned int w = out_edges[k];
//                    if(w == i) continue;
//                    if(in_size>0 && v<w && in_hash.find(w)){
//                        cout<<i<<" "<<v<<" "<<w<<" "<<endl;
//                        num_out_f[k]++;
//                        num_out_f[j]++;
//
//                        unsigned int j2 = binary_search(out_edges,out_pstart[w],out_pstart[w+1],i);
//                        num_out_f[j2]++;
//
//                    }
//                }
...
//
//            unsigned int v = in_edges[j];
//            if(((in_pstart[v+1] - in_pstart[v]) < out_size) || (((in_pstart[v+1] - in_pstart[v]) == out_size) && (i<v))){
//                for(unsigned int k = out_pstart[v]; k<out_pstart[v+1]; k++){
//                    unsigned int w = out_edges[k];
//                    if(w == i) continue;
//                    if(out_size>0 && v<w && out_hash.find(w)){
//                        cout<<i<<" "<<v<<" "<<w<<endl;
//                        num_out_f[binary_search(out_edges,out_pstart[w],out_pstart[w+1],v)]++;
//                        num_out_f[binary_search(out_edges,out_pstart[v],out_pstart[v+1],i)]++;
//                        num_out_f[binary_search(out_edges,out_pstart[i],out_pstart[i+1],w)]++;
//                    }
//                }
//            }
//        }
...
void Graph::getNeighborOrder(){
    ...
    neighbor_order_c2 = new point[edge_m];//申请内存空间
    neighbor_order_f2 = new point[edge_m];
    for(int i = 0;i<n;i++){//对每一个点进行遍历
        
        unsigned int j, out_j;
        
        for(j = pstart[i], out_j = out_pstart[i];j<pstart[i+1];){//遍历i点对应的邻居
            int v = edges[j], out_v = out_edges[out_j];//v为当前的邻居
            if(v<i){//为了避免重复计算
                if(v == out_v){
                    j++;
                    out_j++;
        ...
        sort(neighbor_order_c2+pstart[i], neighbor_order_c2+pstart[i+1],comp3);//进行排序
...
void Graph::getCoreOrder(){
    ...
    core_order_c2 = new point*[max_degree];//申请空间
    core_order_f2 = new point*[max_degree];//申请空间
    max_degree_num = new int[max_degree];//申请空间
    
    int *core_degree = new int[n];
    int *circulate_core = new int[n];
    int circulate_corei = 0;
    
    for(int i = 0;i<n;i++){//进行一遍过滤，找到u=2时对应的点
        core_degree[i] = degree[i];
        if(degree[i] > 0)//去掉度为0的点
            circulate_core[circulate_corei++] = i;
    }
    int circulate = 0;
    int corei, circulate_corei2;
    for(int i = 0;i<max_degree;i++){//核心索引
        //if(i%10 == 0 && i<1000) cout<<i<<endl;
        corei = 0;
        core_order_c2[i] = new point[circulate_corei];//申请每一层索引对应的空间
        core_order_f2[i] = new point[circulate_corei];
        max_degree_num[circulate] = circulate_corei;
        circulate_corei2 = circulate_corei;
        circulate_corei = 0;
        for(int j2 = 0;j2<circulate_corei2;j2++){//将数据记录下来
            ...
            core_degree[j]--;
            if(core_degree[j] > 0) circulate_core[circulate_corei++] = j;//如果去掉当前数据他还有邻居，则可以进入下一次的查找
            corei++;
        }
        sort(core_order_c2[i],core_order_c2[i]+corei,comp3);//排序
...
//        cout<<core_order_c[parameterNum-2][i][1]<<endl;
    ...
    ofstream fout(str+"/output/" + to_string(parameter1) + "-"+to_string(parameter2)+ "-"+to_string(parameterNum) + "-2.txt",ios::out); //创建待写入数据文件
    for(int i = 0;i<clusterNum;i++){
        fout<<"........................"<<i<<"\n";
        for(int j = 0;j<n+1;j++){
            if(cluster[i][j][1] == -1) break;

            fout<<cluster[i][j][0]<<" "<<cluster[i][j][1]<<"\n";
        }
    }
...
void Graph::query2(float parameter1,float parameter2, int parameterNum){
    if(parameterNum-1>max_degree) {
